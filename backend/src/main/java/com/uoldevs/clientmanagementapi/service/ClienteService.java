...
public class ClienteService {
  ...
  public ClienteDto cadastrarCliente(ClienteDto newClienteDto) {
    ...
    Validacoes.validarCpfUnico(newClienteDto.cpf());
    Validacoes.validarEmail(newClienteDto.email());
  ...
  public ClienteDto atualizarCliente(Long id, ClienteDto newClienteDto) {
    ...
    // Verifica se o e-mail informado já está associado a outro cliente
    if (!clienteExistente.getEmail().equals(newClienteDto.email()) && clienteRepository.existsByEmail(newClienteDto.email())) {
      throw new ValidacaoException("E-mail já está associado a outro cliente");
    }

    // Verifica se o CPF informado já está associado a outro cliente
    if (!clienteExistente.getCpf().equals(newClienteDto.cpf()) && clienteRepository.existsByCpf(newClienteDto.cpf())) {
      throw new ValidacaoException("CPF já está associado a outro cliente");
    }
