...
public class ClienteControllerTest {
  ...
  @Test
  public void deveriaRetornarBadRequestAoCadastrarClienteComDadosInvalidos() {
    when(clienteService.cadastrarCliente(any(ClienteDto.class))).thenThrow(new ValidacaoException("Dados inválidos"));

    ResponseEntity<?> resposta = clienteController.cadastrarCliente(new ClienteDto(1, "", "", "", "", ""));
  ...
  @Test
  public void deveriaRetornarBadRequestAoAtualizarClienteComDadosInvalidos() throws ClienteNotFoundException {
    when(clienteService.atualizarCliente(anyLong(), any(ClienteDto.class))).thenThrow(new ValidacaoException("Dados inválidos"));

    ResponseEntity<?> resposta = clienteController.atualizarCliente(1L, new ClienteDto(1, "", "", "", "", ""));

    assertEquals(HttpStatus.BAD_REQUEST, resposta.getStatusCode());
    assertEquals("Dados inválidos", resposta.getBody());
  }
  @Test
  public void deveriaRetornarNotFoundQuandoClienteNaoEncontrado() throws ValidacaoException {
    // Configuração do comportamento do clienteService para lançar uma ValidacaoException
    when(clienteService.getClienteById(anyLong())).thenThrow(new ValidacaoException("Cliente não encontrado"));
